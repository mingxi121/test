...
from KGQA.mianmian_robot import MianRobot
from KGQA.query_poetry import query_author_work
from KGQA.query_disease import query_disease, query_diseases_by_symptoms_and_factors
...
# 8 - 知识图谱问答系统页面 - 睡眠障碍
@app.route('/KGQA_Sleep', methods=['GET', 'POST'])
@login_required  # 登录验证
def KGQA_Sleep():
    return render_template('KGQA_Sleep.html')


# 9 - 基于知识图谱的问答 - 睡眠障碍
@app.route("/KGQA_Sleep_Answer", methods=["POST"])
def KGQA_Sleep_Answer():
    # 古诗词领域问答
    list_str = request.form.get("prompts", None)

    answer_list = json.loads(list_str)
    question = answer_list[-1]['content']  # 获取前端输入的问题: 失眠是什么睡眠障碍的症状啊？
    mainmian_robot = MianRobot(question)
    answer = mainmian_robot.answer()
    answer = answer.replace("\n", "<br>")  # 便于前端展示
    return markdown.markdown(answer)  # 转换成 markdown模式


# 8.5 - 知识图谱问诊页面 - 睡眠障碍
@app.route('/KGQA_Sleep_Inquiry', methods=['GET', 'POST'])
@login_required  # 登录验证
def KGQA_Sleep_Inquiry():
    return render_template('KGQA_Sleep_Inquiry.html')

# 根据症状和因素选项得到睡眠障碍结果 - 睡眠障碍
@app.route("/query_disease_by_symptom_and_factor", methods=["POST"])
def query_disease_by_symptom_and_factor():
    data = request.json
    symptoms = data.get("symptoms", [])
    factors = data.get("factors", [])
    diseases = query_diseases_by_symptoms_and_factors(symptoms, factors)
    return jsonify({"diseases": diseases})


# 10 - 暂定睡眠障碍的搜索 - 睡眠障碍
@app.route('/search_sleep', methods=['GET', 'POST'])
@login_required  # 登录验证
def search_sleep():
    return render_template('search_sleep.html')
...
def search_disease():
    name = request.args.get('name')
    # print(name)
    json_data = query_disease(str(name))
    return jsonify(json_data)
